...
import { getAnsibleGraph } from '~/api/ansibleApi'

export default function AnsibleGraph({ file, width, height }) {
  ...
  useEffect(() => {
    const fetchGraph = async () => {
      try {
        setLoading(true)
        const data = await getAnsibleGraph(file)
        setGraphData(transformGraphData(data))
        setError(null)
      } catch (err) {
        console.error('Error fetching Ansible graph:', err)
    ...
    return (
      <div className="flex items-center justify-center w-full h-full text-white">
        <div className="text-center animate-pulse">
          <p className="text-lg font-semibold">Loading Ansible Graph...</p>
        </div>
      </div>
    )
  }
