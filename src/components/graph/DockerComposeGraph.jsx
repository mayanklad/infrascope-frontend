...
import { getDockerComposeGraph } from '~/api/dockerComposeApi'

export default function DockerComposeGraph({ file, width, height }) {
  ...
  useEffect(() => {
    const fetchGraph = async () => {
      try {
        setLoading(true)
        const data = await getDockerComposeGraph(file)
        setGraphData(transformGraphData(data))
        setError(null)
      } catch (err) {
        console.error('Error fetching Docker Compose graph:', err)
    ...
    return (
      <div className="flex items-center justify-center w-full h-full text-white">
        <div className="text-center animate-pulse">
          <p className="text-lg font-semibold">Loading Docker Compose Graph...</p>
        </div>
      </div>
    )
  }
